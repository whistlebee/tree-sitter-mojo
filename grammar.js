 ...
 * @author tree-sitter-mojo contributors
 * @license MIT
 * @see {@link https://mojolang.org/docs/|Mojo documentation}
 */
...
module.exports = grammar({
  ...
  rules: {
    ...
    convention_parameter: $ => prec.left(PREC.typed_parameter - 1, seq(
    ...
    list_splat_pattern: $ => seq(
      '*',
      choice($.identifier, $.soft_keyword_identifier),
    ),
    ...
    transfer_expression: $ => prec(PREC.transfer, seq(
    ...
    _as_pattern: $ => prec.left(seq($.case_pattern, 'as', field('alias', alias($.expression, $.as_pattern_target)))),

    union_pattern: $ => prec.left(seq(
      $.case_pattern,
      repeat1(seq('|', $.case_pattern)),
    )),

    _complex_pattern: $ => prec.left(seq(
      choice(
        $.class_pattern,
        $.primary_expression,
    ...
    soft_keyword_identifier: $ => prec(-3, alias(
      choice(
        'read',
        'mut',
        'var',
